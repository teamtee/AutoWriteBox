...
npm run test:e2e # 构建前端并用独立临时数据运行真实 Chrome 工作流测试
```
...
2. 在「API 设置」中填入 Base URL、模型名、API Key，保存。Key 在展示时会被掩码。尚未保存的设置会触发页面离开提醒；返回或重新读取需要二次确认才会丢弃草稿。
   ...
   设置页底部的「本次服务模型用量」会显示进程启动以来的调用次数、输入/输出字符、本地估算 token、兼容服务主动返回的 usage token、耗时、失败及最近任务，不保存提示词正文或 Key；估算 token 与供应商 token 会分开显示。可按“API 服务地址 + 模型”登记 USD/CNY 的输入、输出每百万 token 单价；费用优先使用供应商 token、缺失时用本地估算。价格只保存在当前浏览器，不进入作品备份，结果仍是供预算参考的估算值。
