...
export function VersionedBox({ title, versioned, streaming, busy = false, streamingText, size = 'md', onMove, onRewrite, onClear, onSave, onStop }: {
  title: string;
  versioned: Versioned;
  streaming: boolean;
  busy?: boolean;
    ...
    <section className={`vbox vbox-${size} sketch`}>
      <div className="vbox-head">
        ...
        <div className="vbox-tools">
          <button className="hbtn mini" disabled={disabled || !canPrev(versioned)} onClick={() => onMove(-1)}>◀ 上一个</button>
          {streaming
            ? <button className="hbtn mini stop" onClick={onStop}>⏹ 停止</button>
            : <button className="hbtn mini" disabled={busy} onClick={onRewrite}>🔄 重写</button>}
          <button className="hbtn mini" disabled={disabled || !canNext(versioned)} onClick={() => onMove(1)}>下一个 ▶</button>
          {confirmClear
            ? <button className="hbtn mini accent" disabled={disabled} onClick={() => { setConfirmClear(false); onClear(); }}>确认清空？</button>
            : <button className="hbtn mini" disabled={disabled} onClick={() => setConfirmClear(true)}>🗑 删除</button>}
        </div>
      </div>
      {streaming
        ...
        : <textarea className="vbox-body editor" value={draft}
            ...
            onBlur={() => { if (!busy && draft !== cur) onSave(draft); }} />}
