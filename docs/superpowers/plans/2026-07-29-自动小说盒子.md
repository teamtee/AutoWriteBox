...
**Interfaces:**
- Consumes: 无
- Produces: 根 `package.json` 提供脚本 `start`（`node server/index.js`）、`test`（`node --test server/test/`）；`server/index.js` 导出 `createApp()`，返回配置好的 Express 实例（尚无业务路由，仅一个 `GET /api/health` 返回 `{ ok: true }`）。
...
Run: `node --test server/test/` （后端全绿）
